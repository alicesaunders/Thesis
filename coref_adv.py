# -*- coding: utf-8 -*-
"""
Created on Sun Sep  5 15:43:46 2021

@author: Alice Saunders
"""

# code from: https://github.com/allenai/allennlp-models/blob/main/allennlp_models/coref/models/coref.py#L368
...
class CoreferenceResolver(Model):
    ...
    def forward(
        ...
        # added this bit in to view antecedents
        print("predicted_antecedents size {}".format(predicted_antecedents.size()))
        predicted_antecedents_float = predicted_antecedents.float()
        print("predicted_antecedents_float size {}".format(predicted_antecedents_float.size()))
        ...
        output_dict = {
            ...
            # added in float 
            "predicted_antecedents_float": predicted_antecedents_float
        }
        if span_labels is not None:
            ...
            # I added below in 
            output_dict["probs_logs"] = coreference_log_probs
            print("probs_logs size: {}".format(coreference_log_probs.size()))
            
            # TODO - I also added below in
            coreference_probs = util.masked_softmax(coreference_scores, top_span_mask.unsqueeze(-1))
            output_dict["probs"] = coreference_probs
            print("probs size: {}".format(coreference_probs.size()))
         
            coref_prob = torch.flatten(coreference_probs, start_dim=1)
            output_dict["probs_flat"] = coref_prob
            print("probs_flat size: {}".format(coref_prob.size()))
            
            # Adding gold labels into output dict to look at them 
            output_dict['gold_antecedent_labels'] = gold_antecedent_labels
            print('gold_antecedent_labels size: {}'.format(gold_antecedent_labels.size()))
          

        if metadata is not None:
            output_dict["document"] = [x["original_text"] for x in metadata]
        
        # TODO - ADDED IN CLUSTERS FUNCTION (MAKE HUMAN READABLE TO HERE TO ADD CLUSTERS INTO OUTPUT DICT)
        # A tensor of shape (batch_size, num_spans_to_keep, 2), representing
        ...
        print("clusters (batch_clusters) {}". format(batch_clusters))        
        print("clusters size (batch_clusters) {}". format(len(batch_clusters)))  
        
        # make batch_clusters into a tensor object 
        #batch_clusters_tensor = torch.Tensor(batch_clusters)
        #print("batch_clusters_tensor size (batch_clusters) {}". format(batch_clusters_tensor.size()))  
